...
def test_a_generator_config_starts_a_trajectory_the_loop_reports(client):
    ...
    spec, state = ramp["trajectory"]["spec"], ramp["trajectory"]["state"]
    assert spec["tag"] == "ramp" and spec["start"] == 20.0 and spec["to"] == 80.0
    assert abs(spec["start_time_ns"] - start_ns) < 10**9
    assert abs(spec["end_time_ns"] - (start_ns + 180 * 10**9)) < 10**9
    assert spec["rate"] == pytest.approx(60.0 / 180.0)
    assert state == {"to": None, "end_time_ns": None, "rate": None}  # a ramp fixes it all up front
    ...
    spec = regulated["trajectory"]["spec"]
    assert spec["to"] == 50.0 and spec["rate"] == pytest.approx(10 / 60)
