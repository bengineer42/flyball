...
class SteppedClock(Clock):
    ...
    ``now_ns`` is the origin plus whatever ``advance`` has added, so a test
    reads and ticks a rig at exact instants with no sleeping. Anything that
    schedules on real time (``PeriodicLoop``) still runs on real time; drive
    such a rig with ``rig.read(reader)`` by hand instead.
    """
