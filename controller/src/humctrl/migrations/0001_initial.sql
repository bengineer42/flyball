...
--
-- Shape follows the readings model (D-004): a *source* declares *quantities*
-- (each with a fixed unit); a *sample* is every quantity of one source at one
...
-- specific: the quantity names are data.
...
CREATE TABLE session (
    ...
    version     TEXT,                       -- humctrl version string
...
-- Which quantities each source declared. A channel is (source, quantity).
CREATE TABLE channel (
    session_id  INTEGER NOT NULL,
    source_id   INTEGER NOT NULL,
    quantity_id INTEGER NOT NULL,
    PRIMARY KEY (session_id, source_id, quantity_id),
    FOREIGN KEY (session_id, source_id)   REFERENCES source(session_id, id)   ON DELETE CASCADE,
    FOREIGN KEY (session_id, quantity_id) REFERENCES quantity(session_id, id) ON DELETE CASCADE
);
...
-- One value of one sample. The hot table: one row per quantity per sample.
...
CREATE TABLE reading (
    ...
    value       REAL    NOT NULL,
    PRIMARY KEY (session_id, source_id, seq, quantity_id),
...
CREATE INDEX reading_by_channel ON reading (session_id, source_id, quantity_id, offset_ns, value);
