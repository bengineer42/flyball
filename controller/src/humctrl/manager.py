...
class LeasesRequired:
    _required: list[LeasesRequired]

    def __init__(self, *args: LeasesRequired) -> None:
        object.__setattr__(self, "_required", list(args))
...
class NotLeaserError(LeaseError):
    def __init__(self, lease: Lease, leaser: Lease) -> None:
        super().__init__(
            f"Lease {lease.name} is held by {lease.leaser and lease.leaser.name} not held by {leaser.name}"
        )
...
class AlreadyLeasedError(LeaseError):
    def __init__(self, lease: Lease, leaser: Lease) -> None:
        super().__init__(
            f"Lease {lease.name} is already held by {lease.leaser and lease.leaser.name}, cannot be claimed by {leaser.name}"
        )


class Lease:
    _leaser: Lease | None = None
    _leasing: set[Lease]

    def __init__(self, *args: Lease) -> None:
        self._leasing = set(args)

    @property
    def leaser(self) -> Lease | None:
        return self._leaser
    ...
    @property
    def free(self) -> bool:
        return self._leaser is None

    def release(self, leaser: Lease) -> None:
        if self._leaser is not leaser:
            raise NotLeaserError(self, leaser)
        for lease in self._leasing:
            if lease is not self:
                lease.release(leaser)
        self._leaser = None

    def take(self, lease: Lease) -> None:

    def claim(self, leaser: Lease) -> None:
        self._leaser.
        self._leaser = leaser
        self._leasing.append(self)
...
class FractionLeased(Lease):
    pass

class ControllerLease(Lease):
    controller: Controller

    def __init__(self, controller: Controller, fraction_leased: FractionLeased) -> None:
        self._controller = controller
        super().__init__(fraction_leased)

    def on_release(self) -> None:
        self._controller.suspend()


# class FlowLease(Lease):
#     def __init__(self) -> None:
#         self._leases._flows = self
# class PumpLease(Lease):
#     def __init__(self) -> None:
#         self._leases._fraction = self

#     def release(self):
#         self._leases._flows.release(self)

#     def claim(self, owner: Owners) -> None:
