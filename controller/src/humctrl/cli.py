...
Talks to a running ``humctrl-daemon`` over the same HTTP API the browser uses,
...
def request(args: argparse.Namespace, method: str, path: str, body: Any = None) -> Any:
    try:
        ...
        raise CliError(
            f"no daemon at {args.url} (is humctrl.service running?)", EXIT_UNREACHABLE
        ) from None
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="humctrl", description="Control a running humctrl daemon.")
    p.add_argument(
        "--url",
        default=os.environ.get("HUMCTRL_URL", DEFAULT_URL),
        help=f"daemon base URL (env HUMCTRL_URL, default {DEFAULT_URL})",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"flows", "efforts"} and (args.wet is None) != (args.dry is None):
        print(f"humctrl: {args.command} takes both values or neither", file=sys.stderr)
        return EXIT_ERROR
    try:
        args.fn(args)
    except CliError as e:
        print(f"humctrl: {e}", file=sys.stderr)
        return e.code
    except KeyboardInterrupt:
        return 130
    return 0
