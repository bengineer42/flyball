...
class SqliteSessionWriter:
    ...
    def _quantity_id(self, quantity: Quantity, connection: sqlite3.Connection) -> int:
        if (qid := self._quantities.get(quantity)) is None:
            qid = len(self._quantities) + 1
            connection.execute(
                "INSERT INTO quantity (session_id, id, name, unit, label) VALUES (?, ?, ?, ?, ?)",
                (self._session.id, qid, quantity.name, quantity.unit, quantity.label),
            )
            self._quantities[quantity] = qid
        return qid
    ...
    def declare_source(self, source: Source, kind: str | None = None) -> None:
        ...
        with self._store._transaction() as connection:
            ...
            connection.executemany(
                "INSERT INTO channel (session_id, source_id, quantity_id) VALUES (?, ?, ?)",
                [
                    (self._session.id, sid, self._quantity_id(channel.quantity, connection))
    ...
    def write_samples(self, samples: Iterable[Sample]) -> None:
        ...
        for sample in samples:
            ...
            for quantity, value in sample.values.items():
                if (qid := self._quantities.get(quantity)) is None:
                    raise NotDeclaredError("quantity", quantity.name)
        ...
        with self._store._transaction() as connection:
            ...
            connection.executemany(
                "INSERT INTO reading (session_id, source_id, seq, quantity_id, offset_ns, value)"
                " VALUES (?, ?, ?, ?, ?, ?)",
                reading_rows,
            )
...
class SqliteStore:
    ...
    def _quantities(self, session_id: int) -> dict[int, QuantityRow]:
        return {
            r["id"]: QuantityRow(r["id"], r["name"], r["unit"], r["label"])
            for r in self._query("SELECT * FROM quantity WHERE session_id = ?", (session_id,))
        }
    ...
    def channels(self, session_id: int) -> list[ChannelRow]:
        ...
        return [
            ...
            for r in self._query(
                "SELECT source_id, quantity_id FROM channel WHERE session_id = ?"
                " ORDER BY source_id, quantity_id",
                (session_id,),
            )
        ]

    def _channel(self, session_id: int, source: str, quantity: str) -> ChannelRow:
        rows = self._query(
            ...
            " JOIN quantity q ON q.session_id = c.session_id AND q.id = c.quantity_id"
            " WHERE c.session_id = ? AND s.name = ? AND q.name = ?",
            (session_id, source, quantity),
        )
        if not rows:
            raise NotDeclaredError("channel", f"{source}.{quantity}")
        r = rows[0]
        return ChannelRow(
    ...
    def series(
        ...
        channel = self._channel(session_id, source, quantity)
        ...
        base = " FROM reading WHERE session_id = ? AND source_id = ? AND quantity_id = ?" + where
        key = [session_id, channel.source.id, channel.quantity.id, *params]
        ...
        match downsample:
            ...
            case Downsample(max_points=int(max_points)):
                ...
                return self.series(
                    session_id, source, quantity, window, Downsample(bucket_ns=bucket_ns)
                )
