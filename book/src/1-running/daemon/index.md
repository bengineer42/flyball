...
versions, saving, `--resume`. Every setting the daemon takes, as a file
rather than flags: [The daemon section](../../2-config/daemon.md).
...
While nobody is recording, the daemon records everything anyway -- into a
session of its own, the *scratch record* (`kind: "scratch"` in the sessions
list) -- and keeps the last `keep` of it: an hour, unless the
[`daemon:` section](../../2-config/daemon.md) says otherwise. So a chart
...
daemon stays until it ages out, so history survives a restart.

## What ages out

Every 30 seconds of wall time the daemon sweeps the store:
...
The keys, their spellings and defaults: [The daemon section](../../2-config/daemon.md).
...
(or the `daemon:` section) decide where it listens. On shutdown the server stops the
