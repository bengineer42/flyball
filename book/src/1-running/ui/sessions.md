...
**Sessions** (`#/sessions`): start and stop recording, the recorded sessions, and the runner's rolling record. What a session holds and how it is exported over the API: [History](../../4-server/api.md#history); the runner's retention keys: [The runner section](../../2-config/runner.md).
...
(`keep:` in its `runner:` section — see [the runner](../runner/access.md)), the list
also shows that record as one row: "last 58 min held — not a recording",
with what the runner trims it to and how much it holds. It is never a
...
runner's policy in one line — kept, retained, rotated, capped, and where
...
runner does with all this and when: [The scratch record](../runner/index.md#the-scratch-record).
