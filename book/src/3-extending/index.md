...
| --- | --- | --- |
| a device | a `Readable` and/or `Committable` subclass, its tree as descriptors (`Demand`, `Readout`, `Setting`, `Input`) in the class body or built from config, `read` and/or `write_signal`/`commit`, `@command` methods | routes under `/api/devices`, telemetry, CLI subcommands, its signals in the schema |
