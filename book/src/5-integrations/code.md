...
| client | language | |
| --- | --- | --- |
| `flyball.interfaces.client.Rig` | Python | a method per device command synthesised from the schema, `read`, `write`, `watch`, and `get`/`post`/`put` for any route; the runner and MCP server build on it |
