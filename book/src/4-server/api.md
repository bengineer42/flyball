...
actor, written_utc_ns}}`, for a `values` device where each value in force came
from: `rig_file`, `restored` (kept from an earlier run: "restored, written
by `actor` at `written_utc_ns`", wall time) or `written` (in this run) --
