...
| `POST` | `/api/auth/login` | a `password` front: `{"password": "…"}`; a bare runner with a token: `{"token": "…"}`. Sets the session cookie (`HttpOnly; SameSite=Lax`) and answers as `GET`. Wrong: `401` after half a second; ten wrong in a minute from one address: `429` with `Retry-After` (at a bare runner a wrong `Authorization: Bearer` counts toward the ten, and while they stand every bearer request from that address is `429` too); at a front also `429` with `Retry-After: 1` while two other passwords are being checked. A front without the password shape answers `404`. Needs a same-site `Origin` at a front |
