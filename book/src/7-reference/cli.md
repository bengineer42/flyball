...
| key | default | |
| --- | --- | --- |
| `listen` | `127.0.0.1:9000` | the address it serves on. Beyond loopback it logs a warning at start (plain HTTP: its token and every runner's password, token and cookies cross the network in the clear) and will not proxy to an open runner |
