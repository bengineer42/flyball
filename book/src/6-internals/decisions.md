...
| **D-028** | An open runner is never served beyond loopback: the runner refuses to start open on a non-loopback address, and both Go fronts refuse to expose an open runner, unless `insecure_open` is set explicitly; overwriting a rig file keeps its `runner` section | decided |
