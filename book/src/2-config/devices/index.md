...
clears it (`label: null` shows the titlecased name, `warn: null` drops the
...
| `stale_after` | number (seconds) | finite and above zero; checked when a reading is delivered or the controller is regulated; a sensor that stops reporting is not caught. When it trips, a controller regulated from this signal holds its demand rather than apply it. Unset: never checked |
