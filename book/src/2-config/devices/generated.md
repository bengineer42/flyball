...
pydantic model with a `tag` -- and the same class is what validates the rig
...
| the CLI and the client | `GET /api/schema` and `/api/devices/{name}/schema` | `flyball <device> --help`, and a method per command on `flyball.client.Rig` |
...
`bound`, `config` -- belong to the envelope ([Devices](index.md)), and a
driver's config may not declare a field by any of those names: that is
checked when the class is defined, so `signals:` on a `scpi` device is
always the override table, never the query table (which is why that one is
...
A link tag ([Links](../links.md)) is a `Config[Link]` with a `tag`; a law or a
