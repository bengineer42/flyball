...
| `root_path` | `/prefix` | none | `--root-path`, `FLYBALL_ROOT_PATH` | serve everything under a path: `/furnace/api`, `/furnace/ws`, … for several bare runners on one origin -- [a sub-path](../1-running/runner/access.md#a-sub-path). `flyballd` sets it from the manifest's `root_path`. Leave it unset for `flyball run`, whose front serves the rig at `/`: a runner under a prefix fails the front's readiness check, and every request is answered `502` |
