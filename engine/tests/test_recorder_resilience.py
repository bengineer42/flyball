...
def test_a_tick_whose_correction_is_nan_is_kept_not_fatal(fresh):
    ...
    writer.write_tick(Tick(controller.name, 1_000_000_000, "regulating", 5.0, demand=5.0))
    writer.write_tick(Tick(controller.name, 2_000_000_000, "regulating", math.nan, demand=math.nan))
    writer.write_tick(Tick(controller.name, 3_000_000_000, "regulating", 6.0, demand=6.0))
