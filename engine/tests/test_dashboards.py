...
def test_save_read_history_list(client):
    ...
    assert body["body"]["schema_version"] == 4
...
def test_a_version_1_document_is_migrated_to_addresses_and_controllers():
    ...
    assert migrate({"widgets": []})["schema_version"] == 4, "no version is version 1"
...
def test_a_stored_version_1_document_is_migrated_on_read(client, furnace_rig):
    ...
    assert read["body"]["schema_version"] == 4
    ...
    assert [r["body"]["schema_version"] for r in listed] == [4]
    assert c.get("/api/dashboards/old/history").json()[0]["body"]["schema_version"] == 4
...
def test_a_version_2_document_becomes_writable_and_unordered_and_keeps_its_bindings():
    ...
    migrated = migrate(v2)
    assert migrated == {**v2, "schema_version": 4, "readonly": False, "order": None}
...
def test_readonly_and_order_round_trip_and_are_validated(client):
    ...
    assert (body["schema_version"], body["readonly"], body["order"]) == (4, True, 1.5)
