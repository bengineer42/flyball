...
class TestFeedforwards:
    def test_builtins_and_their_configs_round_trip(self):
        ...
        affine = Affine.config_type.model_validate({
            "type": "affine",
            "gain": 2.0,
            "bias": 1.0,
        }).build()
...
class TestController:
    def test_demand_is_feedforward_plus_correction_in_the_target_unit(self):
        ...
        assert loop.output_value == pytest.approx(70.0)
    ...
    def test_delivered_correction_is_measured_from_the_feedforward(self):
        ...
        assert loop.output_value == pytest.approx(190.0)  # 90 + 10 * 10
    ...
    def test_bumpless_seed_subtracts_the_feedforward(self):
        ...
        loop.output_value = loop.expected = 30.0  # what a manual demand left the heater at
    ...
    def test_regulate_at_demand_aims_at_the_setpoint_behind_it(self):
        ...
        assert loop.output_value == pytest.approx(120.0)
        ...
        assert result.output_value == pytest.approx(120.0), (
            "correction reset: demand == feedforward(55)"
        )
