...
_SECRET_KEY = re.compile(r"token|password|secret", re.IGNORECASE)


def redacted(argv: Sequence[str]) -> list[str]:
    """`argv` with the value of each flag that carries a credential replaced by `***`.

    `--token T`, `--password P` (either spelling, `--flag value` or `--flag=value`), and a
    `--set KEY=VALUE` whose key names a token, password or secret. What the lock file, the
    "rig busy" message and the restart log line show of a command line.
    """
    out: list[str] = []
    hide = False
    for arg in argv:
        if hide:
            out.append("***" if out[-1] in _SECRET_FLAGS else _set(arg))
            hide = False
            continue
        flag, eq, _ = arg.partition("=")
        if eq and flag in _SECRET_FLAGS:
            out.append(f"{flag}=***")
        elif eq and flag == "--set":
            out.append(f"--set={_set(arg[len('--set=') :])}")
        else:
            out.append(arg)
            hide = arg in (*_SECRET_FLAGS, "--set")
    return out


def _set(assignment: str) -> str:
    key, eq, _ = assignment.partition("=")
    return f"{key}=***" if eq and _SECRET_KEY.search(key) else assignment
...
def hold(store: Path) -> IO[str]:
    ...
    hardware. The file holds the holder's pid and command line (credentials `redacted`), for
    the message; it is 0600 from its creation.
    """
    ...
    handle.seek(0)
    handle.truncate()
    handle.write(f"pid {os.getpid()}: {' '.join(redacted(sys.argv))}\n")
    handle.flush()
    return handle
