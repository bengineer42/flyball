...
def _stop(rig: Rig, request: Request, reason: str) -> dict[str, Any] | None:
    """The installed stopper's planned stop, before the restart; a failure is logged, not raised.

    Not latched (D-051): a rig edit is a planned change, so the restarted rig comes up
    passive, not stopped.
    """
    ...
    if stopper is None:
        return None
    try:
        return stopper.stop(actor(request), reason, latch=False).as_dict()
    except Exception:
