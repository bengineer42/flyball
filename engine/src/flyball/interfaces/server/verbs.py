...
TABLE: Final[tuple[Rule, ...]] = (  # TODO(D-034): the verbs; the rows are fixed by the routes
    ...
    Rule("GET", "/api/tunings/{name}", READ),
    Rule("PUT", "/api/tunings/{name}", OPERATE),
    ...
    Rule("PUT", "/api/devices/{name}/write", OPERATE),
    Rule("PUT", "/api/signals/{address}", OPERATE),
    Rule("POST", "/api/devices/{name}/commands/{command}", OPERATE),
    ...
    Rule("GET", "/api/activities", READ),
    Rule("GET", "/api/activities/{name}", READ),
    Rule("POST", "/api/activities/{name}/fire", OPERATE),
    Rule("POST", "/api/activities/{name}/cancel", OPERATE),
    ...
    Rule("POST", "/api/programs/cancel", OPERATE),
    Rule("GET", "/api/schema", READ),
    Rule("GET", "/api/sim", READ),
    Rule("PUT", "/api/sim/clock", OPERATE),
    Rule("POST", "/api/sim/clock/advance", OPERATE),
