...
own `[project.entry-points."flyball.configs"]`. `Setpoint`/`NoFeedforward`
live in `flyball.model.feedforward`, not here (`Controller`'s own defaults,
needed a layer below `control`), but are registered from here too -- `model`
is unordered in the layer contract, reachable from everywhere, so importing
them here is no different from `control/feedforward.py` already re-exporting
them for anyone importing the base from its old home.
"""
