...
-- `[{subject_kind, subject}]` (`rig`, `device`, `signal`, `controller`). A row is written
...
-- `actor` is who set it (JSON `{principal, kind, via, sid, message}`: a stop's person
-- or agent, a fault action's controller), `at_ns` when (wall
...
CREATE TABLE latch (
    ...
    actor     TEXT    NOT NULL,              -- JSON {principal, kind, via, sid, message}
