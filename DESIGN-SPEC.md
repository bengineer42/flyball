...
- **17 Sep 2026 — polish pass: text that does not fit, boxes that spill (commits `3c85e28` … `f8bc5ba`).** `scripts/ui-check/overflow.mjs` measures clipped text, boxes past their parent and page-level sideways scroll; three survey agents ran it on every page of eight rigs at 1920/1440/1024/400 and looked at the screenshots. What changed:
  - **Tile headers.** The title keeps its width (up to the head less the actions), the subtitle takes what is left and drops entirely when squeezed to a sliver or in a tile under 230 px; a unit chart's device list ellipsises. Overview tiles have a 14 rem floor so a title like "Chamber temperature" fits at 1920 as at 1440.
  - **Cards and forms in narrow columns.** Device cards put the driver kind under the name and lay their signal table out fixed (26 % label column, so "Supply humidities when unbound" wraps); a faceplate under 300 px stacks label over value; under 60 rem its "regulates …" line moves under the name; segmented choices, sliders and unit inputs wrap; a demand entry wraps. The program widget's event rows keep time and kind whole and let only the message give way; a narrow events tile drops its subject column.
  - **Pages.** Detail pages are titled by label (`Chamber humidity`, not `hum_sensors.chamber.humidity`), and the app-bar title carries its text as a hint. Program steps' descriptions clamp to two lines instead of one ellipsised word; the program text pane scrolls sideways; the check chip never widens the page. Events filters take their own row under the title on a phone. "Drive Drive" is gone on a target named `drive`; a controller whose target has no label is titled like the signal, not by address; the token chip is icon-only on a phone.
  - Verified: humidity, 11 pages × {1440, dark, 1024, 400}: 0 console issues; `overflow.mjs` on every page at 1920/1440/1024/400 reports only ellipsised tile titles in tiles narrower than their words (a 6-column tile at 1024) and MUI's designed 3 px `edge="end"` overhang; `npm test` 72; `tsc` clean; `dash-e2e` ALL PASS on furnace.
  - Not reproduced: a "6/13/29" x-axis date on one furnace chart (polish-a); blank chart widgets in full-page screenshots (polish-c) are charts below the fold, which by design do not draw until scrolled to.
