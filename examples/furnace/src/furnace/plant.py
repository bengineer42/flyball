...
`zone1..N` and `sample`, as the thermocouples read them. A
[MultiPlant][flyball_sim.plant.MultiPlant]: `flyball_sim.devices` shares it
between a `sim_daq` reading the thermocouples and a `sim_drive` setting the
heaters.
"""
...
class Furnace:
    ...
    # endregion
    # region flyball_sim.devices' optional quantity hooks

    def output_quantity(self, port: str) -> Quantity:
