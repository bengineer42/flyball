...
- **Labels come from the server.** Every `label` on the wire -- a device's, a namespace's, a signal's, an input's, a command's, a controller's, a dashboard row's -- is resolved and never empty: the declared one, else the key humanised, lower-case (`dry_pump_flow` → "dry pump flow"). Show it as it comes; `humanise` is only for what has no server label (an event code, a tag value, a dashboard not yet saved).
