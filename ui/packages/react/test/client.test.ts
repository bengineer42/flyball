...
describe("address helpers", () => {
  ...
  it("flattens a device tree to its signals, namespaces recursed", () => {
    const signal = (address: string, access: string): TreeNode =>
      ({ name: address.split(".").pop()!, address, access, label: "", quantity: "q", unit: "", dimension: null, dtype: "float", shape: [], range: null, precision: null, warn: null, alarm: null, poll_s: null, limits: null, together: [], latest: null, write: null }) as TreeNode;
    const tree: TreeNode[] = [
      signal("d.a", "rp"),
