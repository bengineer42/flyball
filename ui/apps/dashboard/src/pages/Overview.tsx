...
import { hrefFor, type Page } from "../router.js";
import { DeviceCard, ReaderCard } from "../cards.js";
import { useReaderRuns } from "./Devices.js";
...
function Stat({ icon: Icon, label, value, tone }: { icon: IconComponent; label: string; value: ReactNode; tone?: Tone }) {
  return (
    <Paper sx={{ display: "flex", alignItems: "center", gap: 1.5, px: 1.5, py: 1, borderColor: tone ? TONE_COLOR[tone] : undefined }}>
...
export function Overview({ schema, sources, traces, states, events, onOpen, windowS, onWindow }: OverviewProps) {
  ...
  return (
    <>
      <div className="stats">
        <Stat icon={h?.ok ? OkIcon : WarnIcon} label="rig" value={h ? (h.ok ? "ok" : "fault") : "…"} tone={h ? (h.ok ? "ok" : "bad") : undefined} />
        <Stat icon={SourceIcon} label="recording" value={h ? (h.recording ? "on" : "off") : "…"} tone={h?.recording ? "ok" : undefined} />
        <Stat icon={PAGE_ICONS.sources} label="readers" value={h ? `${readerCount - readersDown}/${readerCount} running` : "…"} tone={readersDown ? "warn" : undefined} />
        <Stat icon={PAGE_ICONS.loops} label="loops" value={h ? Object.keys(h.loops).length : "…"} />
        <Stat icon={WarnIcon} label="conditions" value={h ? h.conditions.length : "…"} tone={h?.conditions.length ? "warn" : undefined} />
        <Stat icon={CircleIcon} label="signals waiting" value={h ? h.signals.length : "…"} />
        <Stat icon={PAGE_ICONS.events} label="events" value={`${problems} warn/error of ${events.length}`} tone={errors ? "bad" : problems ? "warn" : undefined} />
      ...
      <section>
        <SectionHead
          ...
          end={<WindowSelect value={windowS} onChange={onWindow} />}
        />
        {sources.map((src) => {
        ...
        {sources.length > 0 && (
          <Paper sx={{ p: 1.5 }}>
            <div className="tiles tiles-wide">
              <UnitCharts channels={sources.flatMap((s) => s.channels)} traces={traces} height={160} windowS={windowS} />
            </div>
          </Paper>
        )}
      ...
      <section>
        <SectionHead icon={SourceIcon} title="Readers" />
        <div className="tiles tiles-wide">
          {readers.map((r) => (
